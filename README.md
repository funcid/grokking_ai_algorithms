...
```
В этом разделе реализована простая полносвязная искусственная нейронная 
сеть (ANN) с одной скрытой слоем и сигмоидальной функцией активации. 
Обучение проводится с использованием метода обратного распространения 
ошибки (backpropagation) и градиентного спуска. Архитектура и гиперпараметры 
модели задаются вручную.

Практическое применение — попытка предсказать, приму ли я ванну в 
конкретный день на основе собранных данных за год. В качестве признаков 
использовались события дня (работа, тренажёрный зал, массаж, рестораны и др.). 
Модель обучалась на бинарной классификации (ванна — да/нет), с последующей 
визуализацией предсказаний и анализом влияния каждого признака.

Интересное: Оказалось, что «день с ванной» почти не зависит от других дел.
Градиентный спуск это сдвиг в обратную сторону он вектора стремящимуся к ошибке 
(берем 2 точки, смотрим где Error больше, вот у нас есть delta/производная,
меняем направление, домножаем на Learning Rate и вычитаем это из весов).
```
